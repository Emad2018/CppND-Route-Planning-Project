...
RouteModel::Node *RoutePlanner::NextNode() 
{
   ...
   float f1,f2,f;  
   for (i = 0; i < open_list.size()-1; i++)  
    {  
        // Find the minimum element in unsorted array  
        min_idx = i;       
        for (j = i+1; j < open_list.size(); j++) 
        {
          
          f1=open_list[j]->h_value +open_list[j]->g_value ;
          f2=open_list[min_idx]->h_value+open_list[min_idx]->g_value;
          if ( f1> f2)
          {
             
             min_idx = j; 
             temp=open_list[j];
             open_list[j]=open_list[i];
             open_list[i]=temp;
          }
        }
   }
...
void RoutePlanner::AStarSearch() {
    ...
    {      
      ...
      if((end_node->x==current_node->x)&&(end_node->y==current_node->y))
       {
