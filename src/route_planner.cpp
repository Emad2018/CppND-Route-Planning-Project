...
// - Return the pointer.
static bool Compare(RouteModel::Node *firstNode, RouteModel::Node *secondNode)
{
    return ((firstNode->g_value + firstNode->h_value) > (secondNode->g_value + secondNode->h_value) ? true : false);
}
RouteModel::Node *RoutePlanner::NextNode()
{
    sort(open_list.begin(), open_list.end(), Compare);
    RouteModel::Node *CurrNode = open_list.back();
    open_list.pop_back();
    return CurrNode;
}
